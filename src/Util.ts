...
class Util {
   ...
   */
  static secondsToDifferenceString(
    seconds_input: number,
    { enableSeconds = true }: { enableSeconds: boolean }
  ): string {
  ...
  static log(
    ...
    if (typeof message === 'string') {
      ...
      for (const msg of Discord.Util.splitMessage(message, {
        maxLength: 1980,
      })) {
        client.send({
          content: msg,
          avatarURL: process.showdown.user?.displayAvatarURL(),
          username: 'Showdown! logs',
          files,
        });
      }
    } else
  ...
  static async DeployCommands(): Promise<void | boolean> {
    ...
    const files = await recursive('./cmds').catch((err) =>
      Util.log(`Error while reading commands:\n${err}`)
    );
    ...
    if (process.showdown.user?.id === '1') {
      ...
      await process.showdown.guilds.cache
        .get('709061970078335027')
        ?.commands.set(global.array());
      return Util.log('Application Commands deployed!');
    }
  }
